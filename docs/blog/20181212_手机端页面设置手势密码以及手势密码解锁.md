...
<!--  @/components/pwd.vue -->
<template>
  <div class="masks gesturePwd-wp"
       v-show="currentValue">
    <div class="gesturePwd">
      <div style="position: absolute;top:0;left:0;right:0;bottom:0;">
        <h4 ref="gestureTitle"
            class="gestureTitle">{{textInfo}}</h4>
        <!-- <a ref="updatePassword"
           class="resetPwd"
           @click="updatePassword()">重置密码</a>
        <a ref="updatePassword"
           class="closePwd"
           @click="goAndLogin()">去登陆</a> -->
        <canvas ref="canvas"
                class="canvas"></canvas>
      </div>
    </div>
  ...
  methods: {
    ...
    storePass (psw) {// touchend结束之后对密码和状态的处理
      if (this.pswObj.step == 1) {
        // 设置密码
        if (!this.lengthOk(psw)) {
          return
        }
        if (this.checkPass(this.pswObj.fpassword, psw)) {
          this.pswObj.step = 2;
          this.pswObj.spassword = psw;
          this.textInfo = "密码保存成功"
          this.drawStatusPoint('#2CFF26');
          this.drawPoint('#2CFF26');
          let str = this.getStrPwd(psw)
          this.$emit("setPwdOk", str)
        } else {
          this.textInfo = "两次不一致，重新输入"
          this.drawStatusPoint('red');
          this.drawPoint('red');
          this.reset()
        ...
        // console.log("校验密码中。。。。")
        if (!this.lengthOk(psw)) {
          this.setPleaseUnlock()
          return
        } else {
          this.$emit("checkDone", this.getStrPwd(psw))
        }
      } else {
        // 第一次长度不够
        if (!this.lengthOk(psw)) {
        ...
        this.textInfo = "请再次绘制解锁图案"
        this.reset()
      }
    },
    setPleaseUnlock () {
      console.log('setPleaseUnlock')
      var gestureTitle = this.$refs.gestureTitle;
    ...
    getStrPwd (spwd) {
      let str = ''
      for (let index = 0; index < spwd.length; index++) {
        const el = spwd[index];
        str += el.index + ''
      }
      return str
...
components: {
 ...
 // 添加校验方法
checkPwd (pwd) {
  // console.log("校验密码为：", pwd)
},
// 设置手势密码成功
setPwdOk (item) {
    console.log("已设置手势密码为",item)
    axios
    .post('/crowdsourceUser/updateUser', {
        "openGesturePassword": '1',
        gesturePassword: item
    })
    .then(res => {
        this.showPwd = false
...
//  调用设置手势密码页面
this.$refs.pwd.showSetPwd()
//  调用解锁页面
this.$refs.pwd.setPleaseUnlock()
// <!-- 设置要显示文字 -->
this.$refs.pwd.setTitle("密码错误请重新解锁")
```
